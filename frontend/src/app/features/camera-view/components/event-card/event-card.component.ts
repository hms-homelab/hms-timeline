...
@Component({
  ...
  template: `
    <button
      type="button"
      class="event-card w-full text-left cursor-pointer flex items-center gap-4"
      [class.selected]="isSelected"
      (click)="onPlayClick()"
      [disabled]="!event.recording_url">
      ...
      <div class="flex-shrink-0 w-32 h-24 bg-gray-900 rounded-lg overflow-hidden border border-gray-700">
        @if (event.snapshot_url && event.snapshot_url !== 'http://192.168.2.5:8000/snapshots/None') {
          <img
...
export class EventCardComponent {
  ...
  getSnapshotUrl(): string {
    if (this.event.snapshot_url) {
      try {
        const url = new URL(this.event.snapshot_url);
        return url.pathname.replace(/^\//, ''); // relative: snapshots/filename.jpg
      } catch {
        return this.event.snapshot_url.replace(/^\//, '');
      }
    }
    return '';
  }
